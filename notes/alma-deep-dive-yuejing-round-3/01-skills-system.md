# 01 — Alma skills system (SKILL.md discovery + Skill / SlashCommand tools)
...
Plus a 6th `<workspace>/.alma/skills` when a workspace is open. The
...
Skills exist on disk (read-only as far as alma is concerned), but
...
(round-2 [`06-tool-routing.md`](../alma-deep-dive-yuejing-round-2/06-tool-routing.md))
...
subagent can invoke skills mid-run. The exception is `alma-guide`,
...
agent — alma analyzes completed conversations and decides whether
...
skill." This is the LEARNING LOOP: alma watches its own use, codifies
...
- Does alma's loader detect skills whose path was rewritten (e.g.,
...
- Are there any precedence asymmetries inside the project skill
  scan when a `.alma/skills/foo` shadows the same skill in
